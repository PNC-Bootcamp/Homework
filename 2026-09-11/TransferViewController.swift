...
protocol AccountsRepository {
    ...
    func transfer(amount: Decimal, from: Account, to: Account) async throws -> Void
}
...
struct TransferEligibilityService {
    ...
    func canTransfer(amount: Decimal, from: Account) -> Result<Void, TransferError> {
        ...
        guard from.balance >= amount else { return .failure(.insufficientFunds) }
...
final class TransferViewModel {
    ...
    let eligibility: TransferEligibilityService
    
    init(
        repository: AccountsRepository,
        eligibility: TransferEligibilityService = TransferEligibilityService()
    ) {
    ...
    func attemptTransfer(
        amount amountToTransfer: Decimal,
        from fromAccount: Account,
        to toAccount: Account) async {
            let canTransfer = eligibility.canTransfer(amount: amountToTransfer, from: fromAccount)
            switch canTransfer {
            case .success:
                do { try await repository.transfer(amount: amountToTransfer, from: fromAccount, to: toAccount) }
                catch { }
                onSuccess?()
